...
- **难度三档**(标题页「笔力」):写意 / 工笔 / 狂草,缩放速度曲线 + 墨渍密度/间距。
- **错峰解锁机制**(每章一个,首次遭遇才提示):漂移墨渍(横向游走)→ 减速墨潭(掉速不掉血)→ 侧风阵风(把车横推)→ 独木窄桥(路面收窄穿缝)。
- **正反馈**:完美擦肩(墨渍贴身掠过不撞 → 连击+奖励)、连击 ≥3 拖出渐淡水墨笔触、剪纸阶段按加速滑翔借势。
- **目标 / 重玩**:六章全解锁自由起章、历来最远 + 最高连击纪录、连击 ×5 里程碑、本章零失误回血;**禅意模式**(不死练习,不计纪录)。
